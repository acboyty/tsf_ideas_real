...
from net import RNNNet, RNNGATENet, IDEANet
import numpy as np 
import os
...
class IDEAModel():
    def __init__(self, lookback, lookahead, h_dim, hid_dim, device, data_dir, task):
        self.lookback = lookback
        self.lookahead = lookahead
        self.h_dim = h_dim
        self.hid_dim = hid_dim
        ...
        self.net = IDEANet(self.lookback, self.lookahead, self.h_dim, self.hid_dim).to(self.device)

    def fit(self, X_train, h_train, y_train, X_val, h_val, y_val, metric, max_epoch, patience, batch_size, lr, weight_decay):
        # data loader
        X_train, h_train, y_train = torch.Tensor(X_train), torch.Tensor(h_train), torch.Tensor(y_train)
        X_val, h_val, y_val = torch.Tensor(X_val), torch.Tensor(h_val), torch.Tensor(y_val)
        train_loader = DataLoader(TensorDataset(X_train, h_train, y_train), batch_size=batch_size, shuffle=True)
        # val_loader = DataLoader(TensorDataset(X_val, y_val), batch_size=batch_size, shuffle=True)
        ...
        for epoch in range(max_epoch):
            self.net.train()
            for idx, (X_batch, h_batch, y_batch) in enumerate(train_loader):
                X_batch = X_batch.to(self.device)
                ...
                forecast = self.net(X_batch, h_batch)
                loss = criterion(y_batch, forecast)
            ...
            with torch.no_grad():
                ...
                if loss < best_loss:
                    ...
                    torch.save({
                        'net_state_dict': self.net.state_dict(),
                        'best_epoch': best_epoch,
                    }, os.path.join(self.data_dir, self.task + '.IDEA.th'))
                else:
                    if epoch % 50 == 0:
                        print(f'epoch {epoch:04d} loss: {loss:7.5f} no improvement for {epoch - best_epoch} epoch(es)')
                    if epoch - best_epoch > patience:
    ...
    def eval(self, X_test, h_test):
        checkpoint = torch.load(os.path.join(self.data_dir, self.task + '.IDEA.th'))
        self.net.load_state_dict(checkpoint['net_state_dict'])
        X_test = torch.Tensor(X_test)
        ...
        with torch.no_grad():
            ...
            forecast = self.net(X_test, h_test)
        return forecast.cpu().numpy()