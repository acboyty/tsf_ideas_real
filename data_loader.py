...
def data_loader(data_dir, lookback=20, lookahead=1, input_ts='with_holiday'):
    if 'sin' in data_dir:
        ...
        for st in range(data.shape[1]):
            if st + lookback + lookahead > data.shape[1]:
                break
            X.append(data[:50, st:st + lookback])
            y.append(data[:50, st + lookback:st + lookback + lookahead, 0])
    ...
    elif 'bike-sharing' in data_dir or 'NYC' in data_dir or 'london' in data_dir:
        data = np.load(os.path.join(data_dir, 'data.npy'))
